...
void Absorption::readFolder() {
    vector<string> list = loadtxt<string>(folderPath_ + "/abslist.txt", 1)[0];
...
void Absorption::loadAbs(int ID) {
    ...
    absFilter.wavelength_ = data[0];
    absFilter.bandpass_ = data[1];
    abs_.push_back(absFilter);
}


void Absorption::rescale(const vector<double> &wavelength) {
    for (size_t i = 0; i < abs_.size(); ++i) {
        abs_[i].wavelength_ = wavelength;
        abs_[i].bandpass_ = interp<double>(wavelength,
                                           abs_[i].inputWavelength_,
                                           abs_[i].inputBandpass_,
                                           0.5,
                                           1.0);
    }
}
